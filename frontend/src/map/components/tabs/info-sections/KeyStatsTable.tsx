                    ...
                    <>
                        ...
                        {displayPlace.area_km2 != null && (
                            <span className="block text-[9px] text-muted font-normal mt-0.5 leading-none text-left">
                                {displayPlace.level === "barangay" ? "Estimated from boundary polygon" : "Official area from PSA Table A"}
                            </span>
                        )}
                    </>
                ),
            ...
            <table className="w-full border-collapse">
                <tbody>
                    {table.getRowModel().rows.map((row, index) => (
                        <tr key={row.id} className={cn("border-b border-border last:border-b-0", index % 2 === 0 ? "bg-slate-50/30" : "")}>
