...
export function TopDistributionBar({
    ...
    const chartData = useMemo(() => {
        const validItems = subLevels
            .map((item) => ({
                name: item.name.trim(),
                value: (item as unknown as Record<string, number>)[activeMetric.key] as number | null,
            }))
    ...
    const availableOptions = useMemo(() => {
        return METRICS.filter((opt) => subLevels.some((row) => (row as unknown as Record<string, number>)[opt.key] != null && (row as unknown as Record<string, number>)[opt.key] > 0));
    }, [subLevels]);
